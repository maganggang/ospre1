...
$(function() {
		...
		tools: [{
			...
			},
			{
				...
				handler: function() {
					...
					// var index = $('#index_tabs').iTabs('getTabIndex',
					// $('#index_tabs').iTabs('getSelected'));
					// console.log(index);
	...
	$(".systemName").on("click",
		function(e) {
			...
			// //IE9及以下不兼容data-menuid属性
			// generateMenu(e.target.getAttribute('data-menuid'),
	...
	$(".nav-right-btn").on('click',
		function() {
			navsScrollTo("next");
		});
	$(".nav-left-btn").on('click',
		function() {
			navsScrollTo("previous");
		});
	$(".nav-group").on("click", 'li',
		function(e) {
			// 添加选择样式
	...
	// 显示系统首页
	/*
	 * setTimeout(function () { var indexTab = []; indexTab.iconCls =
	 * "icon-house"; indexTab.text = "系统门户"; var portal =
	 * $.getUrlParam("portal"); if (portal == "system" || portal == null) portal =
	 * "system"; indexTab.url = "html/portal/index.html"; indexTab.closable =
	 * false; indexTab.border = false; addTab(indexTab); }, 1);
	 */
...
function tabMenuOprate(menu, type) {
	...
	$.each(allTabs,
		function(i, n) {
			...
			if(opt.closable) allTabtitle.push(opt.title);
		});
	...
	switch(type) {
		...
		case "3":
			// 除此之外全部关闭
			for(var i = 0; i < allTabtitle.length; i++) {
				if(curTabTitle != allTabtitle[i]) $('#index_tabs').iTabs('close', allTabtitle[i]);
			}
...
function generateMenu(menuId, systemName) {
	...
	if(menuId < 8000) {
		...
		//var url = "/json/menu/menu_" + menuId + ".json";
		var url = "/simple1/authority/selectMyAuthById";
		if(topJUI.config.appendRefererParam) url = appendRefererParam(url);
		$.get(url, {
				"parentId": ""
			},
			...
			function(data) {
				...
				$.each(data,function(i, e) { // 循环创建手风琴的项
					if(e){
						...
						// $.parser.parse();
						var secondUrl = url+"?parentId="+e.id;
						if(topJUI.config.appendRefererParam) secondUrl = appendRefererParam(secondUrl);
						$.get(secondUrl,
							function(data) { // 循环创建树的项
								$("#tree" + e.id).tree({
									...
									onBeforeExpand: function(node, param) {
										var thirdUrl =url+"?parentId="+node.id;
										if(topJUI.config.appendRefererParam) thirdUrl = appendRefererParam(thirdUrl);
										$("#tree" + e.id).tree('options').url = thirdUrl;
									},
									onClick: function(node) {
